...
-- Úplný poèet pøidaných filmù a seriálù v datasetu
...
WHERE 
	type IS NOT NULL 
	AND type <> 'William Wyler' -- Nepøesná data
GROUP BY type;


-- Úprava sloupce 'date_added'
...
-- Úplný poèet pøidaných filmù a serialù každý rok
...
-- TOP 5 nejaktivnìjších režisérù
...
-- Top 10 nejdelších filmù pro dospìlé (TV-MA rating)
...
-- TOP 10 zemí s nejvìtší rozmanitostí hercù a hereèek ve svých filmech/seriálech
...
-- Prùmìrná délka filmù v jednotlivých zemích pro malé dìti (TV-Y rating)
...
-- Odstranìní pøebyteèných tabulek
